...
exports.executeTransaction = async ({
  ...
  const lockset = await client.set(
    redisKey,
  ...
  try {
    ...
    if (sourceAccount.status !== "active")
      throw new Error("Please reach out to customer care rep for help");
    ...
    if (sourceAccount.currency !== destinationAccount.currency)
      throw new Error(
        `Invalid transaction type: can only transfer to ${sourceAccount.currency} account`
      );
    ...
    const newSourceAccountBalance =
      Number(sourceAccount.balance) - Number(amount);
    ...
    return {
      ...
      from: sourceAccount.full_name,
      to: destinationAccount.full_name,
    };
  } catch (e) {
...
exports.getTransactions = async (payload) => {
  ...
  const { rows, count } = await Transaction.findAndCountAll({
    ...
    attributes: { exclude: ["idempotency_key", "updatedAt"] },
  });
  return { rows, count };
};
